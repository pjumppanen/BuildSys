...
\abstract{%
A powerful feature of R is the ability to make use of dynamically loaded libraries and call functions within (API) through the \code{.C} or \code{.CALL} interface. 
This allows the more numerically intensive parts of an algorithm to be implemented in efficient compiled code (typically C or FORTRAN), 
obtaining a significant boost in performance. The flip side of this approach is the complexity and difficulty of compilation, and inparticular,
the debugging of the compiled code. \pkg{BuildSys} is a package that streamlines and integrates into R, the process of both building and debugging
...
are typically handled through the \code{R CMD SHLIB}\citep{SHLIB} mechanism. Internally that mechanism typically relies on \code{GNU make}\citep{GnuMake} which 
in turn will typically use \code{GCC}\citep{gcc} (on most operating systems) or \code{CLANG/LLVM}\citep{LLVM} (on MacOS). Compilation through \code{R CMD SHLIB} 
...
invoking the \code{R CMD SHLIB} command. Furthermore, if the shared library contains many individual source files and makes use of a number of static
...
and GNU make invocation directly rather than going through the \code{R CMD SHLIB} mechanism. \pkg{BuildSys} takes this approach by encapsulating 
...
tools to do so. The most commonly used debugger is \code{GDB}\citep{GDB}, which provides realtime debugging of the dynamic library within the
...
command line based debugger, \code{LLDB}\citep{LLDB}. 

If users manage to build the library with appropriate debug information included, load the hosting R session and have the debugger find and 
...
one major issue remains that has to do with inspection of variables and other data structures. By default, both \code{GDB} and \code{LLDB} only 
...
requires the use of \code{.C} or \code{.Call} mechanisms directly; see for instance \citep{DotC} and \citep{DotC2}. This requires an intimate
...
use a C++ library that encapsulates the native R data structures within complex data types, of which both \code{GDB} and \code{LLDB} are  
incapable of visualising in a meaningful way. The most widely used interface wrapper within the R community is \pkg{Rcpp}\citep{RcppIntro},\citep{Rcpp}. 
The packages \pkg{RcppEigen} and \pkg{TMB} build upon the interface that \pkg{Rcpp} provides to support linear algebra\citep{RcppEigen}, 
